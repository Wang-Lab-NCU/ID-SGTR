...
class TokenConfig:
    ...
    # --- 通用文本限制 ---
    MAX_CANDIDATE_POOL = 20           # 传给 LLM 的 Next Hop 候选节点最大数量
...
class ID_SGTR_Reasoning_Engine:
    ...
    def _get_top_chunks(self, candidate_cids, query_vec, current_context_id=None, top_k=5, min_score=0.25):
        """基于余弦相似度过滤并排序候选文本 Chunk，支持单一 Context 物理隔离"""
        
        # ==========================================
        # 🛡️ 1. 物理隔离墙：直接提取唯一目标 Context 下的合法 Chunk
        # ==========================================
        if current_context_id is not None:
            # O(1) 极速提取：直接从 JSON 映射字典中拿出这个 context 下所有的 chunk_id
            allowed_cids = set(str(c) for c in self.chunk_dict_by_ctx.get(str(current_context_id), []))
            
            # 过滤：必须即在候选列表里，又在允许的 Context 里，且存在于向量库中
            valid_cids = [
        ...
        valid_cids = list(dict.fromkeys(valid_cids)) # 保持顺序的去重
    ...
    def step2_semantic_anchoring(self, query, query_vec, top_k=25):
        """
        语义锚定 4.0 (Context-First Routing)
        1. 全局召回实体 -> 2. 反查候选 context_id -> 3. 向量打分锁定唯一最强 context -> 4. 过滤实体 -> 5. LLM 精准狙击
        """
        with self.gpu_lock:
            # 1. 🌍 始终以全局模式去知识库捞取较多的候选实体
            df = self.matcher.link(query, context_id=None, top_k=top_k)            
            if df.empty: return [], set()

        # 2. 溯源：从图谱中反向收集这些实体涉及的所有候选 context_id
        candidate_ctxs = set()
        ent_to_ctxs = {}
        ...
        # 3. 🥇 核心突破：对每个候选 Context 下的 Chunks 进行向量打分，决出最强 Context
        ctx_scores = {}
        ...
        for ctx in candidate_ctxs:
            ...
            # 复用 _get_top_chunks 的逻辑，矩阵化批量打分
            ...
            # ==========================================
            # 🎯 取 Top 5 的 Chunk 得分求平均，计算该 Context 的"证据密度"
            # ==========================================
        ...
        # 4. 🔒 终极锁定：只取分数最高的唯一的 context_id！
        best_ctx = max(ctx_scores, key=ctx_scores.get)
        active_contexts = {best_ctx}
        print(f"  🎯 [Context Routing] 成功锁定目标语境包: {best_ctx} (Top-5 平均最高相似度: {ctx_scores[best_ctx]:.3f})")

        # 5. 过滤实体：只保留属于这个最优 context 里的实体
        filtered_entities_by_ctx = []
        for ent, ctxs in ent_to_ctxs.items():
            if best_ctx in ctxs:
                filtered_entities_by_ctx.append(ent)
            if len(filtered_entities_by_ctx) >= 15: # 防 LLM 窗口溢出
                break
                
        if not filtered_entities_by_ctx:
            filtered_entities_by_ctx = list(ent_to_ctxs.keys())[:5] # 兜底

        # 6. 获取文本定义，交给 LLM 进行最后的筛选
...
"""
    ...
    def step3_iterative_agent_reasoning(self, seeds, query, active_contexts, intent_weights, query_vec, max_hops=3, verbose=True, max_prompt_chunks=TokenConfig.MAX_CHUNKS_IN_PROMPT):
        """核心推理 Agent，参数改为接收 active_contexts 集合"""
...
"""
    ...
    def _expand_neighbors(self, source_nodes, query_vec, active_contexts, intent_weights, top_k_per_node=None, verbose=False, visited_set=None):
        """向外扩展，受 active_contexts 防火墙严格限制"""
            ...
            if add_chunks and ((not has_def) or TokenConfig.STAGE0_ADD_CHUNKS):
                if n in self.G:
                    for nbr in self.G.neighbors(n):
                        ...
                        # 🏠 防火墙机制：如果有 active_contexts，只拉取该范围内的 Chunk
                        if active_contexts is not None:
                            edge_ctxs = {str(c) for c in edge_data.get('context_ids', set())}
                            if not edge_ctxs.intersection(active_contexts):
                                continue # 噪音边，拦截！
...
if __name__ == "__main__":
    ...
    #     # 👇 请加上这两行，让我看看它的真实面貌
    #     print("\n🔍 [诊断] chunk_df 列名:", df_chunk.columns.tolist())
    ...
    # 2. 初始化引擎 (MASK_RATIO 为消融实验预留，默认为 0)
