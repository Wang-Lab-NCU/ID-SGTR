...
# Controls the context window size fed into LLM and similarity truncation to prevent token explosion or noise
# ==========================================
class TokenConfig:
    # --- Stage 0: initial definition retrieval ---
    STAGE0_ADD_CHUNKS = True          # Whether to forcibly attach relevant text chunks during entity definition phase
    STAGE0_MAX_CHUNKS = 3            # Maximum number of chunks to pass in the initial stage
    
    # --- Stage N: path expansion ---
    TOP_K_NEIGHBORS = 12              # Maximum number of neighbor nodes explored per step in graph by the agent
    CHUNK_SIM_THRESHOLD_STRICT = 0.35 # Cosine similarity threshold for semantically supplemented chunks (higher threshold, noise prevention)
    CHUNK_SIM_THRESHOLD_LOOSE = 0.25  # Cosine similarity threshold for structurally associated chunks (lower threshold, guaranteed by graph edges)
    MIN_EDGE_SCORE = 0.05             # Minimum comprehensive edge weight, below which the edge is considered disconnected
    
    # --- General text limits ---
    MAX_CANDIDATE_POOL = 20           # Maximum number of candidate nodes for Next Hop passed to LLM
    CHUNK_CHAR_LIMIT = 1000           # Character truncation length for a single text chunk (prevents overly long text)
    MAX_CHUNKS_IN_PROMPT = 3          # Maximum total chunks allowed per prompt assembly
...
class ID_SGTR_Reasoning_Engine:
    def __init__(self, 
        ...
        # ✅ Defensive programming: force all input IDs to be converted to string
        ...
        def parse_vec_safe(x):
            """Safely parse vector: compatible with numpy, list, and string representation of array"""
    ...
    def _build_hybrid_graph(self, graph_df, proximity_df):
        ...
        # ✅ Defensive programming: also force all IDs in graph input to string
        df_g = graph_df.copy()
        ...
        if self.edge_mask_ratio > 0.0:
            print(f"\n⚠️ [Ablation Study] Randomly dropped {dropped_count}/{total_explicit} ({dropped_count/total_explicit*100:.1f}%) explicit edges!\n")

      # 2. Attach implicit edges (co-occurrence relations)
        if proximity_df is not None and not proximity_df.empty:
            ...
            for row in df_p.itertuples(index=False):
                ...
                if G.has_edge(u, v):
                    # If edge already exists (either previously attached explicit edge or implicit edge built from another context)
                    ...
                    G[u][v]['context_ids'].add(ctx_id) # 👈 core: inject current context_id into the edge's pass
                    if chunks:
    ...
    def _get_top_chunks(self, candidate_cids, query_vec, current_context_id=None, top_k=5, min_score=0.25):
        """Filter and rank candidate text chunks based on cosine similarity, supporting physical isolation of a single context"""
        ...
        if current_context_id is not None:
            # O(1) ultra-fast extraction: directly take all chunk_ids under this context from the JSON mapping dictionary
            allowed_cids = set(str(c) for c in self.chunk_dict_by_ctx.get(str(current_context_id), []))
            
            # Filter: must be both in candidate list and allowed context, and exist in vector database
            valid_cids = [
        ...
        try:
            # ==========================================
            # 🧠 2. Vector extraction and similarity calculation
            # ==========================================
            ...
            # ==========================================
            # ⚡ 3. Efficient sorting and threshold truncation
            # ==========================================
    ...
    def step1_analyze_intent(self, query, query_vec):
        """Intent analysis: reuse global query vector"""
        with self.gpu_lock:
            try:
    ...
    def step2_semantic_anchoring(self, query, query_vec, top_k=25):
        ...
        1. Global recall of entities -> 2. Reverse lookup candidate context_ids -> 3. Vector scoring to lock the single strongest context -> 4. Filter entities -> 5. LLM precise sniper
        """
        with self.gpu_lock:
            # 1. 🌍 Always use global mode to retrieve many candidate entities from the knowledge base
            df = self.matcher.link(query, context_id=None, top_k=top_k)            
            if df.empty: return [], set()

        # 2. Trace back: from the graph, collect all candidate context_ids involved by these entities
        candidate_ctxs = set()
        ent_to_ctxs = {}
        ...
        # 3. 🥇 Core breakthrough: vector score each candidate context's chunks to determine the strongest context
        ctx_scores = {}
        ...
        for ctx in candidate_ctxs:
            ...
            # Reuse logic from _get_top_chunks, batch scoring with matrix
            ...
            # ==========================================
            # 🎯 Take average score of top 5 chunks as "evidence density" of this context
            # ==========================================
        ...
        if not ctx_scores:
            # Fallback: if no scores, revert to old logic
            return list(ent_to_ctxs.keys())[:5], set()

        # 4. 🔒 Ultimate lock: take only the single context_id with the highest score!
        ...
        # 5. Filter entities: keep only those belonging to this best context
        ...
        # 6. Get text definitions and let LLM perform final filtering
...
"""
    ...
    def step3_iterative_agent_reasoning(self, seeds, query, active_contexts, intent_weights, query_vec, max_hops=3, verbose=True, max_prompt_chunks=TokenConfig.MAX_CHUNKS_IN_PROMPT):
        """Core reasoning agent, parameter modified to accept active_contexts set"""
        ...
        for hop in range(1, max_hops + 1):
            ...
            for p in candidate_paths:
                if self.G.has_edge(p['u'], p['v']):
                    ...
                    # 🏠 Structural chunk extraction also subject to firewall restrictions
            ...
            for node in current_focus_nodes:
                if node in self.G:
                    for nbr in self.G.neighbors(node):
                        ...
                        # 🏠 Semantic chunk extraction also subject to firewall restrictions
                        if not active_contexts or edge_ctxs.intersection(active_contexts):
...
"""
    ...
    def _parse_llm_decision(self, text, valid_scope=None):
        """Robust regex parsing and format fallback, including false positive (False Positive) interception"""
        text = str(text).strip()
    ...
    def _fallback_answer(self, query, active_contexts, query_vec, seed_infos=None, verbose=False):
        ...
        # Fallback: if no chunks found in locked scope, fall back to global scoring
...
"""
    ...
    def _expand_neighbors(self, source_nodes, query_vec, active_contexts, intent_weights, top_k_per_node=None, verbose=False, visited_set=None):
        """Expand outward, strictly restricted by active_contexts firewall"""
        ...
        for u in source_nodes:
            ...
            for v in self.G.neighbors(u):
                ...
                # 🏠 Firewall intercepts physical traversal routes
                if active_contexts is not None:
                    ...
                    if not edge_ctxs.intersection(active_contexts):
                        continue # If not in the dynamically locked document, this path is blocked!
            ...
            if add_chunks and ((not has_def) or TokenConfig.STAGE0_ADD_CHUNKS):
                if n in self.G:
                    for nbr in self.G.neighbors(n):
                        ...
                        # 🏠 Firewall mechanism: if active_contexts is set, only fetch chunks within that scope
                        if active_contexts is not None:
                            edge_ctxs = {str(c) for c in edge_data.get('context_ids', set())}
                            if not edge_ctxs.intersection(active_contexts):
    ...
    def solve(self, query, context_id=None, verbose=True, mode='full'):
        """Main engine entry point, seamlessly switch between Global and Local"""
        ...
        if not seeds: 
            return "Sorry, no relevant entities found in the knowledge graph.", "No-Seeds"
...
# ==========================================
# 4. Main program entry point (multi-threaded test processing)
# ==========================================
if __name__ == "__main__":
    ...
    # 1. Data loading and fault tolerance protection
    try:
    ...
    def process_query_wrapper(i: int, row: pd.Series) -> Tuple[int, Any]:
        ...
        # Pass to core engine for parsing
    ...
    output_path = os.path.join(current_dir, "query_results_agent_10Qwen3-8B_4_15_global.csv")
