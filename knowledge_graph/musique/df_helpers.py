...
def build_enriched_name_text(row):
    ...
    Used to improve the accuracy of vector retrieval.
    """
...
# ======================================================================
# 2. Entity merging logic (new - from merge.py)
# ======================================================================

def merge_concepts(df: pd.DataFrame) -> pd.DataFrame:
    """
    Merge entities with the same (context_id, Standard_Entity), combine descriptions and synonyms.
    """
    print("🔄 [Helper] Merging entity data...")
    df = df.copy()
    ...
    # 2. Add 'Entity' (original mention) to synonym candidates
    ...
    df_grouped = df.groupby(['context_id', 'Standard_Entity', 'category'], as_index=False).agg({
        'chunk_id': 'first',  # Take the first chunk_id as representative
        'cluster_id': 'first',
        'category': 'first',
        'description': merge_descriptions,
        'synonyms': merge_lists,
    })
    # [Modification] Ensure aggregated data is sorted by chunk_id
    df_grouped = df_grouped.sort_values(by=['chunk_id']).reset_index(drop=True)
    
    print(f"✅ [Helper] Merge complete. Original rows: {len(df)} -> After merge: {len(df_grouped)}")
    return df_grouped
...
def ExtractConcepts(dataframe: pd.DataFrame, model=None, max_workers: int = 100) -> pd.DataFrame:
    ...
    def concept_extractor(i, row):
        ...
        # If a 429 error or JSON error occurs here, parallel_llm_processor will catch and retry.
    ...
    start_msg = f"--- Step: Starting parallel extraction of concept nodes from {len(dataframe)} chunks ---"
...
def ResolveCoreferences(dataframe: pd.DataFrame, model=None, max_workers=15) -> pd.DataFrame:
    ...
    def coref_resolver(i, row):
        # resolve_coreferences no longer swallows 429 errors
    ...
    # If completely fails (after 3 retries), fallback to original text
...
def df2Graph(dataframe: pd.DataFrame, entity_map: dict, model=None, max_workers=100) -> list:
    ...
    start_msg = f"--- Step: Starting parallel graph relation extraction ---"
    successful_results = parallel_llm_processor(
        dataframe=dataframe,
...
def graph2Df(nodes_list) -> pd.DataFrame:
    ...
    # [Modification] Ensure the relation table is sorted by chunk_id
...
def contextual_proximity(df: pd.DataFrame) -> pd.DataFrame:
    ...
    dfg2 = (
        # ✅ Add context_id_1 to the grouping key to prevent cross-article merging
    ...
    # ✅ Fix: Rename columns strictly in the order generated by reset_index()
    dfg2.columns = ["context_id", "node_1", "node_2", "chunk_id", "count"]
    
    dfg2["context_id"] = dfg2["context_id"].astype(str)
    
    # Now the "count" column is indeed numeric; comparison will no longer raise errors
