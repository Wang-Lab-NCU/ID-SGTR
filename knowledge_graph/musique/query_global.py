...
# ==========================================
# 0. Helper class: console color output (for debug log differentiation)
# ==========================================
class Colors:
    HEADER = '\033[95m'
    BLUE = '\033[94m'
...
# ==========================================
# 1. Token optimization configuration
# Controls the context window size and similarity cutoff to avoid token explosion or noise
# ==========================================
class TokenConfig:
    # --- Stage 0: initial entity definition check ---
...
class ID_SGTR_Reasoning_Engine:
    ...
    def _get_top_chunks(self, candidate_cids, query_vec, current_context_id=None, top_k=5, min_score=0.25):
        """Filter and rank candidate text chunks based on cosine similarity, supporting physical isolation by a single Context"""
        
        # ==========================================
        # 🛡️ 1. Physical isolation wall: directly extract legal chunks from the only target Context
        # ==========================================
        if current_context_id is not None:
            # O(1) fast extraction: from the JSON mapping dict get all chunk_id for this context
            allowed_cids = set(str(c) for c in self.chunk_dict_by_ctx.get(str(current_context_id), []))
            
            # Filter: must be both in candidate list and in allowed Context, and exist in the vector DB
            valid_cids = [
        ...
        valid_cids = list(dict.fromkeys(valid_cids)) # keep order while deduplicating
    ...
    def step2_semantic_anchoring(self, query, query_vec, top_k=25):
        ...
        1. Global entity recall -> 2. Back-check candidate context_ids -> 3. Vector scoring to lock the single strongest context -> 4. Filter entities -> 5. LLM precise selection
        """
        with self.gpu_lock:
            # 1. 🌍 Always retrieve a larger set of candidate entities from the knowledge base in global mode
        ...
        # 3. 🥇 Core breakthrough: score each candidate Context by vector similarity of its chunks, determine the strongest Context
        ctx_scores = {}
        ...
        for ctx in candidate_ctxs:
            ...
            # Reuse the logic of _get_top_chunks, matrixized batch scoring
            ...
            # ==========================================
            # 🎯 Take the average score of top 5 chunks to compute the "evidence density" of this Context
            # ==========================================
        ...
        # 4. 🔒 Final lock: keep only the single context_id with highest score!
        ...
        # 5. Filter entities: keep only those belonging to this optimal context
        ...
        # 6. Get text definitions, let LLM do the final filtering
...
"""
    ...
    def step3_iterative_agent_reasoning(self, seeds, query, active_contexts, intent_weights, query_vec, max_hops=3, verbose=True, max_prompt_chunks=TokenConfig.MAX_CHUNKS_IN_PROMPT):
        """Core reasoning agent, modified to accept active_contexts set"""
        ...
        for hop in range(1, max_hops + 1):
            ...
            for p in candidate_paths:
                if self.G.has_edge(p['u'], p['v']):
                    ...
                    # 🏠 Structure chunk extraction also respects the firewall
...
"""
    ...
    def _fallback_answer(self, query, active_contexts, query_vec, seed_infos=None, verbose=False):
        ...
        # Fallback: if locked scope yields no chunks, revert to global scoring
...
"""
    ...
    def _expand_neighbors(self, source_nodes, query_vec, active_contexts, intent_weights, top_k_per_node=None, verbose=False, visited_set=None):
        ...
        for u in source_nodes:
            ...
            for v in self.G.neighbors(u):
                ...
                # 🏠 Firewall intercepts physical walking paths
                if active_contexts is not None:
                    ...
                    if not edge_ctxs.intersection(active_contexts):
                        continue # If not in the dynamically locked article, this path is blocked!
    ...
    def solve(self, query, context_id=None, verbose=True, mode='full'):
        """Main engine entry point, seamlessly switches between Global and Local"""
...
# ==========================================
# 4. Main program entry (multi-threaded test processing)
# ==========================================
if __name__ == "__main__":
    ...
    # 1. Data loading with fault tolerance
    try:
    ...
    # 2. Initialize engine (MASK_RATIO reserved for ablation study, default 0)
...
# ==========================================
# 4. Main program entry (automated batch test version)
# ==========================================
# if __name__ == "__main__":
...
#     # 1. Basic data loading (only once)
#     try:
#         df_qa = pd.read_csv(QA_FILE, sep="|")
#         df_graph = pd.read_csv(GRAPH_FILE, sep="|")
#         
...
#         sys.exit(1)
# 
#     # 2. Basic engine initialization (load LLM and vector model, only once)
#     print("🚀 Initializing ID-SGTR engine core components...")
#     engine = ID_SGTR_Reasoning_Engine(
#         intent_model_path=MODEL_FILE,
#         parquet_path=PARQUET_FILE,
#         graph_df=df_graph,
#         chunk_df=df_chunk,
#         proximity_df=df_prox,
#         edge_mask_ratio=0.0  # initially 0
#     )
# 
#     # =====================================================================
#     # 🌟 Automated evaluation configuration area
#     # =====================================================================
#     target_data = df_qa.head(1000)  # range of dataset to test
#     
#     # Remember to run two sets of comparison experiments!
#     # First set: set to 'explicit_only' (test Baseline)
#     # Second set: set to 'full' (test our Hybrid algorithm)
#     RUN_MODE = 'full' 
#     
...
#         print(f"\n\n{'='*80}")
#         print(f"🔥 [Round {mask_ratios.index(mask_ratio) + 1}/5] Testing MASK_RATIO = {mask_ratio} ({mask_pct}%) | Mode: {RUN_MODE}")
#         print(f"{'='*80}")
#         
#         # [Core optimization] Dynamically rebuild graph structure (no need to reload large models, extremely fast)
#         print("🕸️ Applying MASK and regenerating graph structure...")
...
#             dataframe=target_data,
#             processing_func=process_query_wrapper,
#             start_message=f"Starting multi-threaded inference (MASK={mask_pct}%)...",
#             max_workers=5,
#             max_retries=6,
#             initial_delay=2
#         )
...
#         gc.collect()
# 
#     print("\n🎉🎉🎉 All 5 rounds of MASK testing completed! Ready to wrap up.")
