...
class KGPipeline:
    ...
    # =================================================================
    # 3. 其他原有流程 (Splitting, Standardization, etc.)
    # =================================================================
    def load_and_split_data(self, json_path, max_contexts=None, chunk_size=300, chunk_overlap=50):
        ...
        print(f"🚀 [Pipeline] 开始加载 2WikiMultiHopQA 数据: {json_path}")
        ...
        for i, item in enumerate(data[:limit]):
            # 2Wiki 的 context 字段是一个列表，包含多个段落
            # 格式: [ ["Title1", ["Sent1", "Sent2"]], ["Title2", ["Sent3"]] ]
            context_list = item.get("context", [])
            
            for paragraph in context_list:
                # 容错处理：有时格式可能是 [Title, [Sentences]]
                ...
                # 将句子列表合并成一段完整的文本
                # 2Wiki 的句子通常已经是分割好的，直接拼接
                paragraph_text = "".join(sentences)
                
                # 如果文本太长，这里可以使用 self.splitter 进行再次切分
                # 但通常 2Wiki 的段落不长，直接作为一个 Chunk 即可
                
                # 构造 Chunk
                chunks.append({
                    "context_id": i,          # 关联到第 i 个问题
                    "chunk_id": global_chunk_id,
                    "title": title,
                    "text": f"{title} information: {paragraph_text}" # 加上标题增强语义
                })
                global_chunk_id += 1
    ...
    def extract_qa_pairs(self, json_path, max_contexts=None):
        print(f"🚀 [Pipeline] 提取 2WikiMultiHopQA QA 对...")
        with open(json_path, "r", encoding="utf-8") as f:
            data = json.load(f)
        ...
        for i, item in enumerate(data[:limit]):
            ...
            # 2Wiki 的 answer 通常直接是字符串，但有时为了保险起见转一下
