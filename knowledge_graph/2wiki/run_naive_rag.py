...
class Naive_Vector_RAG_Engine:
    def __init__(self, chunk_df, device=None):
        """
        Initialize the pure vector retrieval engine
        """
        ...
        # -------------------------------------------------------
        print("📦 [1/3] Data preprocessing and Context index building...")
        ...
        # Build context_id to chunk_id index (fallback for local retrieval if needed)
        ...
        # -------------------------------------------------------
        # Module 3: Load large language model for inference (LLM)
        # -------------------------------------------------------
        print("🤖 [3/3] Initializing inference LLM (Qwen3-8B)...")
        self.llm = get_chat_model(task_type="kg_query")

    def _get_top_chunks_dense_only(self, candidate_cids, query_vec, top_k=3):
        """
        Pure dense vector similarity computation over the whole index
        """
        ...
        try:
            # 1. Extract content vector matrix (when doing global retrieval, this extracts all chunks)
    ...
    def solve(self, query, context_id=None, verbose=False, top_k=3):
        ...
        # 1. Vectorize the query (lock to prevent CUDA crashes)
        query_vec = None
        with self.gpu_lock:
            try:
        ...
        # 2. ✅ [Core change]: Get candidate Chunk IDs, completely lift isolation!
        candidate_cids = []
        if context_id is None or str(context_id).lower() == 'global':
            # 🌍 Global mode: retrieve all chunk IDs from the entire knowledge base
            candidate_cids = self.chunk_df.index.tolist()
        elif str(context_id) in self.chunk_dict_by_ctx:
            # 🏠 Local mode (only kept for backward compatibility)
            candidate_cids = self.chunk_dict_by_ctx[str(context_id)]
        else:
            # Fallback to full base
            candidate_cids = self.chunk_df.index.tolist()

        # 3. Pure vector retrieval Top‑K over entire base (default 3)
        ...
        # 5. Build prompt (strictly restrict answering based on provided references)
        prompt = f"""
...
if __name__ == "__main__":
    ...
    def process_query_wrapper(i: int, row: pd.Series):
        q = row['question']
        ctx = row['context_id'] # keep for logging comparison
        gold = row['answer']
        
        # ✅ [Core change]: force pass 'global', completely discard original Context isolation!
    ...
    processed_results = parallel_llm_processor(
        ...
        start_message="🚀 Starting multi‑threaded pure vector reasoning...",
        max_workers=5,
        max_retries=6,
        initial_delay=2
    )
